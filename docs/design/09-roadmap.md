...
- 99.9% gateway/control-plane availability under PDB + multi-AZ chaos test — exit is the **chaos matrix in Section 3.4** passing in full (every row's pass condition green), not a single happy-path probe.
...
### 3.4 Chaos-test exit matrix (the 99.9% criterion, made falsifiable)

The "multi-AZ chaos test" exit gate is not one experiment; it is the matrix below run as a **scheduled CI job** against the production-shaped staging cluster. Each experiment injects a single, named fault, the test asserts the **expected SLO behavior**, and the run only counts as a pass if **both** universal pass conditions hold *throughout and after* the fault: (1) the >=10,000 hostile-probe **leakage gate stays green** (a fault must never open a cross-tenant path), and (2) **bit-exact replay still holds** — a `ContextBundle` rendered during the fault window replays to the identical `rendered_prompt_hash` from its per-tenant-encrypted bundle (degradation may change *which* backend served the request, never the determinism of ContextOS's own stages).

**Chaos tool: [LitmusChaos](https://litmuschaos.io/)** (CNCF, Kubernetes-native). Chosen because experiments are declarative `ChaosEngine` CRDs that run *inside* the cluster as a GitHub Actions / Argo step, so every experiment is version-pinned, reviewable, and **CI-gated** — the same fault runs identically on every release candidate. **Rejected alternative: manual failover drills only (a runbook an SRE executes by hand)** — non-repeatable (depends on who runs it and how carefully), not CI-gated (cannot block a merge or a release), and untestable for the two universal pass conditions because nobody fires 10k hostile probes by hand mid-drill. We also rejected Chaos Mesh (comparable capability, but LitmusChaos's ChaosHub experiment catalog and native Argo/Actions hooks fit our existing CI without a second control-plane operator) and AWS FIS alone (cloud-control-plane faults like AZ loss only — it cannot kill a specific Deployment or saturate Redis Streams at the app layer, so it cannot exercise rows 2 and 4).

| # | Experiment (single injected fault) | Expected SLO behavior | Quantitative bound | Universal pass conditions |
| --- | --- | --- | --- | --- |
| 1 | **Kill 1 AZ** (`node-drain` + `pod-network-loss` across one of the >=3 AZs holding gateway replicas) | Stay within **99.9%** availability: with >=3 stateless gateway replicas across >=3 AZ and PDB `minAvailable=2`, surviving replicas absorb traffic; no request error spike beyond the error budget. Control overhead stays **< 250 ms p95**. | Error-rate over the experiment window <= the 0.1% monthly budget pro-rated to the window; p95 control overhead < 250 ms; recovery (lost AZ's pods rescheduled) within KEDA/scheduler reconcile, no manual step. | Leakage gate green; bit-exact replay holds. |
| 2 | **Kill the embedding Deployment** (scale the Embedding service — Deployment #3 — to 0 / `pod-delete` all replicas) | **Fail-open to BM25-only** retrieval with bounded **recall loss <= 12%** (the canonical degraded mode, D1.1/D3.4). No 5xx from the absence of embeddings; memory retrieval still returns within **< 100 ms p95** on the lexical path; semantic cache misses fall through, never error. | Recall delta vs. baseline <= 12% on the fixed eval set; retrieval p95 < 100 ms on BM25-only; 0 requests failed *because* the embedder was down (requests fail-open, not fail-closed). | Leakage gate green; bit-exact replay holds (BM25-only bundles replay bit-exact). |
| 3 | **Force Postgres failover** (promote replica / kill primary via `pod-kill` on the primary StatefulSet member) | Reads/writes resume on the promoted primary within the **<= 30 s failover budget**. RLS (`FORCE ROW LEVEL SECURITY` + `SET LOCAL app.tenant_id`) and partitioning survive failover unchanged. In-flight transactions roll back cleanly (no torn writes); the **fail-closed cost outbox (C12) drops no committed cost record** — unacked outbox rows are redelivered post-failover, billing stays exact. | Time-to-first-successful-write after primary kill <= 30 s; 0 committed cost-ledger records lost (outbox reconciliation count matches pre-fault committed count); 0 RLS-policy gaps post-promotion. | Leakage gate green; bit-exact replay holds. |
| 4 | **Saturate Redis Streams** (inject backpressure / `pod-io-stress` + flood the async plane consumer that doubles as the replay log) | The **async** plane degrades gracefully: best-effort sampled traces (C12) may shed load (fail-open + sampled), but the **billing-grade cost path drops nothing** (durable outbox, retry/DLQ) and the **replay log loses no committed bundle**. The **synchronous** hot path (gateway -> assembly -> backend) is unaffected — write-back is async, so saturation adds latency to consolidation, not to the < 250 ms control path. | Sync-path p95 control overhead unchanged (< 250 ms) under stream saturation; 0 dropped cost records (outbox + DLQ reconcile to zero loss); replay-log entry count for committed requests == request count. | Leakage gate green; bit-exact replay holds. |

**Run cadence & gating:** the full matrix runs **nightly** against staging and as a **required pre-release job** on every M3 release candidate; a single failing row blocks the release exactly as the leakage gate blocks a merge. The matrix is additive to, never a relaxation of, Section 6's invariants — chaos may degrade ContextOS to a *correct, coarser* mode (BM25-only retrieval, sampled traces) but may never breach isolation or replay determinism.
